...
static process_t *getNextProcess() {
	...
	do {
		if (scheduler->processes[current] != NULL &&
			...
			scheduler->processes[current]->remainingQuantum =
...
int64_t addProcess(mainFunction main, char **argv, char *name,
	...
	for (newPid = 0; newPid < MAX_PROCESSES; newPid++) {
		if (scheduler->processes[newPid] == NULL)
			break;
	}
	uint16_t parentPid =
		(scheduler->current != NO_PID) ? scheduler->current : NO_PID;

	process_t *newProcess = createProcessStructure(
		newPid, parentPid, NO_PID, main, argv, name, MIN_PRIORITY, unkillable,
...
int32_t killProcess(uint16_t pid) {
	...
	if (scheduler->processes[pid] == NULL)
		return -1;
	if (scheduler->processes[pid]->unkillable)
		return -1;
...
static void adoptChildren(int16_t pid) {
	for (int i = 0; i < MAX_PROCESSES; i++) {
		if (scheduler->processes[i] != NULL &&
			scheduler->processes[i]->parentPid == pid) {
			scheduler->processes[i]->parentPid = 0;
		}
	}
}
...
int64_t waitPid(uint32_t pid) {
	...
	if (scheduler->processes[pid] == NULL)
		return -1;
	if (scheduler->processes[pid]->parentPid != scheduler->current)
		return -1;
...
void my_exit(int64_t retValue) {
	...
	if (parent != NULL && parent->status == BLOCKED &&
		parent->waitingForPid == currentProcess->pid) {
		unblockProcess(parent->pid);
	}
	yield();
}
