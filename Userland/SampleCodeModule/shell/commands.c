...
static command_t commands[] = {
	...
	{"phylo", "Starts the philosophers program.", 0,
	 (mainFunction) &philosopherProgram},
	{"tsy", "Tests the synchronization primitives. Usage: tsy [n] [use_sem]", 0,
	 (mainFunction) &test_sync},
	{"cat", "Prints stdin as received.", 0, (mainFunction) &cat},
	{"loop",
	 "Prints its PID every certain number of ticks. Usage: loop [ticks]", 0,
	 (mainFunction) &loop},
	{"filter", "Filters vowels and spaces from stdin.", 0,
	 (mainFunction) &filter},
	{"wc", "Counts the words in stdin.", 0, (mainFunction) &wc},
	{"block", "Blocks a process. Usage: block [pid]", 1, &block_command},
	{"unblock", "Unblocks a process. Usage: unblock [pid]", 1,
	 &unblock_command},
	{"nice", "Changes the priority of a process. Usage: nice [pid] [priority]",
	 1, &nice_command},
...
static const char *regNames[REGS_AMOUNT] = {
	...
	"R10", "R11", "R12", "R13", "R14", "R15", "RIP", "RSP"};
		...
		for (int j = 0; j < COMMANDS_COUNT; j++) {
			if (strcmp(executable_commands[i].command, commands[j].name) == 0) {
				...
				if (commands[j].builtin) {
					return commands[j].function(executable_commands[i].argc,
												executable_commands[i].args);
				}
				else {
					executable_commands[i].pid = _sys_createProcess(
...
static int inforegCommand(int argc, char *argv[]) {
	...
	if (!ok) {
		printError("inforeg", "Registers are not updated. Use CTRL + R to update.",
				   NULL);
		return ERROR;
	}
...
static int nice_command(int argc, char *argv[]) {
	...
	if (priority < MIN_PRIORITY || priority > MAX_PRIORITY) {
		printError("nice", "Invalid priority. Must be between 1 and 4.",
				   "nice [pid] [priority]");
		return ERROR;
	}
